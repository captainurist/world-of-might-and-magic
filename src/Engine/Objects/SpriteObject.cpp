...
void SpriteObject::createSplashObject(Vec3i pos) {
    ...
    if (objID != -1) {
        pAudioPlayer->playSound(SOUND_splash, Pid::item(objID), 0, 0, 0);
    }
}
    ...
    switch (object->uType) {
        ...
        case SPRITE_SPELL_DARK_TOXIC_CLOUD: {
            ...
            if (PID_TYPE(pid) != OBJECT_Item) {
                ...
                //            v124, 0, -1, 0, v97, 0, 0);
                pAudioPlayer->playSpellSound(object->uSpellID, Pid::item(uLayingItemID), true);
                return 0;
            }
            updateSpriteOnImpact(object);
            if (object->uObjectDescID == 0) {
            ...
            object->spellSpriteStop();
            pAudioPlayer->playSound(SOUND_fireBall, Pid::item(uLayingItemID));
            return 0;
        }
        ...
        case SPRITE_OBJECT_EXPLODE: {  // actor death explode
            ...
            object->spellSpriteStop();
            pushAoeAttack(Pid::item(uLayingItemID), engine->config->gameplay.AoeDamageDistance.value(),
        ...
        case SPRITE_SPELL_WATER_ICE_BLAST: {
            ...
            //        - 1] + 1; pAudioPlayer->playSound((SoundID)v125, v124, 0,
            //        -1, 0, v16, 0, 0);
            pAudioPlayer->playSpellSound(temp.uSpellID, Pid::item(uLayingItemID), true);
            return 0;
        }
        ...
        case SPRITE_SPELL_DARK_SHRINKING_RAY:  {
            ...
            LOBYTE(v102) = Pid::item(uLayingItemID);
            pSpriteObjects[uLayingItemID].vVelocity.x = 0;
            pSpriteObjects[uLayingItemID].vVelocity.y = 0;
            pSpriteObjects[uLayingItemID].vVelocity.z = 0;
...
void applySpellSpriteDamage(unsigned int uLayingItemID, Pid pid) {
    ...
    if (PID_TYPE(pid) == OBJECT_Character) {
        ...
        DamageCharacterFromMonster(Pid::item(uLayingItemID), pSpriteObjects[uLayingItemID].spellCasterAbility, &velocity, -1);
    } else if (PID_TYPE(pid) == OBJECT_Actor) {
        velocity = pSpriteObjects[uLayingItemID].vVelocity;
        normalize_to_fixpoint(&velocity.x, &velocity.y, &velocity.z);
        switch (PID_TYPE(pSpriteObjects[uLayingItemID].spell_caster_pid)) {
            case OBJECT_Actor:
                Actor::ActorDamageFromMonster(Pid::item(uLayingItemID), PID_ID(pid), &velocity, pSpriteObjects[uLayingItemID].spellCasterAbility);
                break;
            case OBJECT_Character:
                Actor::DamageMonsterFromParty(Pid::item(uLayingItemID), PID_ID(pid), &velocity);
                break;
            case OBJECT_Item:
                ItemDamageFromActor(Pid::item(uLayingItemID), PID_ID(pid), &velocity);
                break;
            default:
                break;
        }
    ...
    for (uint i = 0; i < pSpriteObjects.size(); ++i) {
        if (pSpriteObjects[i].uAttributes & SPRITE_SKIP_A_FRAME) {
            ...
            if (pSpriteObjects[i].uObjectDescID) {
                ...
                processSpellImpact(i, Pid::item(i));
            }
        }
    }
}
